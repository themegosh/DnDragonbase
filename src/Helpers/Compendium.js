...
class Compendium {
    ...
    SetSource(url) {
        return fetch(url, {
            ...
            .then(str => {
                var tree = new XmlToJs().parseXML(str).compendium;
                ...
                tree.race.forEach(race => {
                    this.Races.push(race);
                });
                tree.background.forEach(bg => {
                    this.Backgrounds.push(bg);
                });
                tree.feat.forEach(feat => {
                    this.Feats.push(feat);
                });
                tree.monster.forEach(monster => {
                    this.Monsters.push(monster);
                });
                tree.spell.forEach(spell => {
                    this.Spells.push(spell);
                });
                tree.class.forEach(aClass => {
                    this.Classes.push(this.MapClass(aClass));
                });
    ...
    MapClass(aClass) {
        ...
        aClass.autolevel.forEach(level => {
            //console.log(level);
            ...
            if (typeof level.feature !== "undefined") {
                ...
                levels[lvlNum].features.push(level.feature);
            }

            //spell slots
            if (typeof level.slots !== "undefined") {
                if (typeof level.slots === "string") {
                    //csv slots (could be ',' or ', ')
                    levels[lvlNum].slots = level.slots.split(",").map(s => {
                        return s.trim();
                    });
                } else {
                    //optional slots are in subobjects
                    levels[lvlNum].optionalSlots = true;
                    levels[lvlNum].slots = level.slots["#text"]
                        .split(",")
                        .map(s => {
                            return s.trim();
                        });
    ...
    Search(query, isDeep) {
        ...
        this.Items.forEach(item => {
            if (
                ...
                (isDeep && this.TextArrContains(regex, item.text))
            ) {
        ...
        this.Spells.forEach(spell => {
            if (
                spell.name.search(regex) > -1 ||
                (isDeep && this.TextArrContains(regex, spell.text))
            ) {
                results.Spell.push(spell);
            }
        });
        ...
        this.Feats.forEach(feat => {
            if (
                feat.name.search(regex) > -1 ||
                (isDeep && this.TextArrContains(regex, feat.text))
            ) {
                results.Feat.push(feat);
            }
        });
        ...
        this.Races.forEach(race => {
            if (
                race.name.search(regex) > -1 ||
                (isDeep && this.TraitContains(regex, race.trait))
            ) {
                results.Race.push(race);
            }
        });
        ...
        this.Backgrounds.forEach(background => {
            if (
                background.name.search(regex) > -1 ||
                (isDeep && this.TraitContains(regex, background.trait))
            ) {
                results.Background.push(background);
            }
        });
        ...
        this.Monsters.forEach(monster => {
            if (
                monster.name.search(regex) > -1 ||
                (isDeep && this.TraitContains(regex, monster.trait))
            ) {
                results.Monster.push(monster);
            }
        });
